...
public class UserAuthenticationSystem {
    ...
    private static void saveUsers() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(userFilePath))) {
            for (User user : users) {
                writer.write(user.getUsername() + "_" + user.getPassword() + "_" + user.getFullName() + "_"
                        + user.getDateOfBirth() + "_" + user.getInstitute() + "_" + user.getDegree() + "_"
                        + user.getCgpa() + "_" + user.getPhoneNumber() + "_" + user.getEmail() + "_" + user.getAddress());
                writer.newLine();
            }
        } catch (IOException e) {
